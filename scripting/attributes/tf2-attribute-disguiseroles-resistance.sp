...
//Defines
#define ATTRIBUTE_NAME "disguise roles damage resistance bonus"
...
{
	name = "[TF2-Weapons] Attribute :: Disguise Roles Damage Resistance Bonus", 
	author = "Drixevel", 
	description = "Grants bonuses or penalties for damage resistance based on the role of the spy disguise.", 
	version = "1.0.0", 
...
{
	if (TF2_GetPlayerClass(victim) != TFClass_Spy || !TF2_IsPlayerInCondition(victim, TFCond_Disguised))
	...
	switch (TF2_GetClassRole(view_as<TFClassType>(GetEntProp(victim, Prop_Send, "m_nDisguiseClass"))))
	{
		case TFRole_Offense:
		{			
			if (g_Setting_Offensive_Buff[melee] > 0.0)
			{
				damage = FloatDivider(damage, g_Setting_Offensive_Buff[melee]);
				changed = true;
			}
		}
		case TFRole_Defense:
		{
			if (g_Setting_Defensive_Buff[melee] > 0.0)
			{
				damage = FloatDivider(damage, g_Setting_Defensive_Buff[melee]);
				changed = true;
			}
		}
		case TFRole_Support:
		{
			if (g_Setting_Support_Buff[melee] > 0.0)
			{
				damage = FloatDivider(damage, g_Setting_Support_Buff[melee]);
				changed = true;
			}
		}
	}
